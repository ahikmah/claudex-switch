...
const CANCELLATION_SIGNALS = ['SIGINT', 'SIGTERM'];
const ONLINE_ERROR_CODES = new Set([
  'network-error',
  'online-check-failed',
...
function createProfileLifecycle(options = {}) {
  ...
  const onlineHealthCheck = typeof options.onlineHealthCheck === 'function'
    ? options.onlineHealthCheck
    : () => ({
      status: 'unknown',
      errorCode: 'online-check-unavailable',
    });
  ...
  function inspectProfilesWithCandidates() {
    const report = {
    ...
    if (activeState) {
      ...
      if (activeIsValid && activeState.credentialCount === 0 && activeState.errorCodes.length === 0) {
        ...
        activeState.credentialFile = activeCandidate.file;
    ...
    for (const state of states) {
      ...
      if (!state.active) {
        if (state.localValid) state.credentialFile = state.candidates[0].file;
        state.status = state.localValid ? 'ready' : 'invalid';
      }
    }
    ...
    return {
      report,
      onlineProfiles: states
        .filter((state) => state.credentialFile && ['active', 'ready'].includes(state.status))
        .map((state) => ({
          profile: report.profiles.find((profile) => (
            profile.name === state.name && profile.active === state.active
          )),
  ...
  function inspectProfiles() {
    return inspectProfilesWithCandidates().report;
  }
  ...
  function onlineResultCode(result, fallback) {
    const code = result && typeof result === 'object'
      ? result.errorCode || result.code
      : null;
    return ONLINE_ERROR_CODES.has(code) ? code : fallback;
  }

  function normalizeOnlineResult(result) {
    const rawStatus = typeof result === 'string'
      ? result
      : result && typeof result === 'object' ? result.status : null;
    const status = typeof rawStatus === 'string' ? rawStatus.toLowerCase() : String(rawStatus);
    if (status === 'valid') {
      return { status: 'valid' };
    }
    if (['rejected', 'unauthorized', 'forbidden', '401', '403'].includes(status)) {
      return {
        status: 'needs-reauth',
        code: 'credential-rejected',
      };
    }
    if (['expired', 'token-expired'].includes(status)) {
      return { status: 'needs-reauth', code: 'credential-expired' };
    }
    if (status === 'network-error') return { status: 'unknown', code: 'network-error' };
    return {
      status: 'unknown',
      code: onlineResultCode(result, 'online-check-failed'),
    };
  }

  function runOnlineHealthCheck(candidate) {
    try {
      return normalizeOnlineResult(onlineHealthCheck({
        profile: candidate.profile,
        credentialFile: candidate.credentialFile,
      }));
    } catch (error) {
      const rawErrorCode = error && typeof error === 'object' ? error.code : null;
      const errorCode = typeof rawErrorCode === 'string' ? rawErrorCode.toLowerCase() : String(rawErrorCode);
      if (['credential-rejected', 'invalid-credential', 'unauthorized', 'forbidden', '401', '403'].includes(errorCode)) {
        return { status: 'needs-reauth', code: 'credential-rejected' };
      }
      if (['credential-expired', 'expired', 'token-expired'].includes(errorCode)) {
        return { status: 'needs-reauth', code: 'credential-expired' };
      }
      if (['network-error', 'enotfound', 'econnrefused', 'econnreset', 'etimedout', 'eai_again'].includes(errorCode)) {
        return { status: 'unknown', code: 'network-error' };
      }
      return { status: 'unknown', code: 'online-check-failed' };
    }
  }

  function addOnlineIssue(report, profile, result) {
    if (!profile.errorCodes.includes(result.code)) profile.errorCodes.push(result.code);
    if (report.issues.some((issue) => issue.code === result.code && issue.profile === profile.name)) return;
    const message = result.status === 'needs-reauth'
      ? `Online check rejected the Credential for Profile '${profile.name}'. Reauthentication is required.`
      : `Online check could not confirm the Credential for Profile '${profile.name}'.`;
    report.issues.push({ code: result.code, profile: profile.name, message });
  }

  function onlineDoctorReport() {
    const inspection = inspectProfilesWithCandidates();
    for (const candidate of inspection.onlineProfiles) {
      const result = runOnlineHealthCheck(candidate);
      if (result.status === 'valid') continue;
      const profile = candidate.profile;
      profile.status = result.status;
      addOnlineIssue(inspection.report, profile, result);
    }
    return inspection.report;
  }

  function doctor(io, json, online) {
    const report = online ? onlineDoctorReport() : inspectProfiles();
    if (json) {
      io.stdout(JSON.stringify(report));
    } else {
  ...
  function normalizeLegacyGeneratedEntries(transaction) {
    if (
      !transaction
      || !['add', 'reauth'].includes(transaction.operation)
      || !Array.isArray(transaction.generatedCredentialPaths)
      ...
      return { transaction, ignored: false };
    }

    const loginDirectory = recoveryLoginDirectory(transaction);
    if (!loginDirectory.valid) return { transaction, ignored: false };
    const allowedDirectories = new Set([
      path.resolve(activeDir),
      path.resolve(loginDirectory.directory),
    ]);
    const generatedCredentialPaths = [];
    const generatedCredentialIdentities = [];
    let ignored = false;

    for (const [index, candidate] of transaction.generatedCredentialPaths.entries()) {
      if (recoveryMoveKind(candidate) === 'credential') {
        generatedCredentialPaths.push(candidate);
        generatedCredentialIdentities.push(transaction.generatedCredentialIdentities[index]);
        continue;
      }
      if (
        typeof candidate !== 'string'
        || !path.isAbsolute(candidate)
        || !allowedDirectories.has(path.dirname(path.resolve(candidate)))
        || !recoveryPathState(candidate).safe
      ) {
        return { transaction, ignored: false };
      }
      ignored = true;
    ...
    if (!ignored) return { transaction, ignored: false };
    return {
      transaction: {
        ...transaction,
  ...
  function repairTransaction(repairs) {
    ...
    const normalized = normalizeLegacyGeneratedEntries(transaction);
    transaction = normalized.transaction;
    let terminalRecord = ['committed', 'rolled-back'].includes(transaction.status);
    let safeRecord = terminalRecord
      ? transaction.status === 'committed'
        ? transactionHasSafeCommittedRecord(transaction)
        : transactionHasSafeRolledBackRecord(transaction)
      : transactionHasDeterministicRollback(transaction);
    let recordIsRolledBack = transaction.status === 'rolled-back';
    if (!safeRecord && normalized.ignored && transaction.status === 'recovery-required') {
      const rolledBackTransaction = {
        ...transaction,
        status: 'rolled-back',
        cleanupReady: true,
      };
      if (transactionHasSafeRolledBackRecord(rolledBackTransaction)) {
        transaction = rolledBackTransaction;
        terminalRecord = true;
        recordIsRolledBack = true;
        safeRecord = true;
      }
    }
    if (!safeRecord) {
    ...
    if (normalized.ignored) {
      repairs.push({
        code: 'legacy-generated-entry-ignored',
        target: 'Profile recovery transaction',
        message: 'A legacy non-Credential login artifact was ignored during recovery.',
      });
    }
    if (terminalRecord || recordIsRolledBack) {
      try {
        removeTransaction();
      } catch {
        return {
  ...
  function createCancellationError(signal) {
    return createCommandError(
      'Profile mutation cancelled.',
      signal === 'SIGTERM' ? 143 : 130,
    );
  }

  function installCancellationHandlers(onSignal) {
    if (!signalSource || typeof signalSource.on !== 'function') return () => {};
    const removeListener = typeof signalSource.removeListener === 'function'
      ? (signal, handler) => signalSource.removeListener(signal, handler)
      : typeof signalSource.off === 'function'
        ? (signal, handler) => signalSource.off(signal, handler)
        : null;
    if (!removeListener) return () => {};

    const handlers = CANCELLATION_SIGNALS.map((signal) => {
    ...
    return () => {
      for (const { signal, handler } of handlers) removeListener(signal, handler);
    };
  }
  ...
  function makeGeneratedCredentialPrivate(file) {
    const stats = filesystem.lstatSync(file);
  ...
  function captureGeneratedCredentials(transaction, originalFiles, directory = activeDir) {
    ...
    for (const entry of entries) {
      ...
      if (!stillOriginal.has(file)) {
        ...
        makeGeneratedCredentialPrivate(file);
        generated.push(file);
      }
    }
    return generated;
  ...
  function runWithProxyAvailable(work, startWhenStopped) {
    ...
    if (wasRunning || (startWhenStopped && !operationError)) {
      try {
        proxyService.start();
      } catch (restartError) {
        if (operationError) {
  ...
  function ensureProxyAvailable(startWhenStopped) {
    if (startWhenStopped && proxyService && !proxyService.isRunning()) proxyService.start();
  }

  function startsProxyWhenStopped(command, prepared) {
    return command === 'use'
      || command === 'add'
      || (command === 'reauth' && prepared.targetIsActive);
  }
  ...
  function runMutation(command, force, prepare, operation) {
    ...
    const removeCancellationHandlers = installCancellationHandlers((signal) => {
      if (!cancellationSignal) cancellationSignal = signal;
    });
    const assertNotCancelled = () => {
      if (cancellationSignal) throw createCancellationError(cancellationSignal);
    };

    try {
      releaseLock = operationLock.acquire(command);
      assertNotCancelled();
      assertNoRecovery();
      ...
      const startWhenStopped = startsProxyWhenStopped(command, prepared);
      if (prepared && prepared.noop) {
        ensureProxyAvailable(startWhenStopped);
        return prepared;
      }

      assertNotCancelled();
      transaction = createTransaction(command, prepared);
      return runWithProxyAvailable(() => {
        ...
        try {
          ...
          if (cancellationSignal) {
            ...
            throw createCancellationError(cancellationSignal);
          }
          throw error;
        }
      }, startWhenStopped);
...
Command rules:
  list, current, and doctor read local storage. doctor is offline by default.
  doctor --online is read-only. doctor --repair changes only deterministic local state.

Options:
  --json: stable JSON output (schemaVersion: 1) for list, current, and doctor only.
  --online: doctor only; run a read-only online Provider check for local Credentials.
  --repair: doctor only; apply deterministic local repairs and one safe recovery result.
  --force: Profile mutations and doctor --repair only; bypass only the session warning.
  --yes: delete only; skip the exact Profile-name confirmation.
  --allow-account-change: reauth only; allow a changed Provider account email.

Profile names:
  Profile names start with a letter or number and use only letters, numbers, dot, underscore, or hyphen.
  Profile names are unique without case differences. Reserved command names are rejected.
  Email addresses may repeat across Profiles; exact duplicate Credential data is rejected.
...
Online diagnosis:
  doctor --online checks each locally valid Credential with the online health adapter.
  A rejected or expired Credential is needs-reauth. Provider or network uncertainty is unknown.

Safety and recovery:
  Profile mutations and doctor --repair use a private operation lock and recovery record.
  A running Claudex or proxy session causes a session warning. --force bypasses only that warning.
  Ctrl-C and SIGTERM cancel mutations and roll them back. A failed rollback or process crash blocks later mutations until doctor --repair.
  delete removes only local Profile and Credential files. Remote Provider accounts are never deleted.
...
Status meanings: active is the selected healthy Profile; ready is an inactive healthy Profile;
  needs-reauth is a rejected or expired online Credential; invalid is unsafe or incomplete local state;
  unregistered is an active Credential without a matching Profile; unknown is an online check result that is not known.
  ...
  function run(argv, io = { stdout: console.log, stderr: console.error }) {
    try {
      ...
      if (!command || command === 'help' || command === '--help' || command === '-h') {
        ...
        if (parsed.online) throw createCommandError('--online can be used only with doctor.', 2);
      ...
      if (parsed.online && command !== 'doctor') {
        throw createCommandError('--online can be used only with doctor.', 2);
      }
      if (command === 'list') {
        if (parsed.force) throw createCommandError('--force can be used only with a Profile mutation.', 2);
      ...
      if (command === 'doctor') {
        if (parsed.positional.length !== 1) throw createCommandError('Invalid command. Use --help for usage.', 2);
        if (parsed.online && parsed.repair) {
          throw createCommandError('--online cannot be used with --repair.', 2);
        }
        if (parsed.repair) return repairProfiles(io, parsed.json, parsed.force);
        if (parsed.force) throw createCommandError('--force can be used only with a Profile mutation.', 2);
        return doctor(io, parsed.json, parsed.online);
      }
      if (parsed.json) {
